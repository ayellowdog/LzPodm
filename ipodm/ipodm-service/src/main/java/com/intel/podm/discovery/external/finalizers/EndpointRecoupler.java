...
class EndpointRecoupler extends Recoupler<Endpoint> {
	private static final Logger logger = LoggerFactory.getLogger(EndpointRecoupler.class);
    ...
    protected void reattach(ComposedNode node, Collection<Endpoint> assets) {
        ...
        if (endpointToReattach != null) {
            ...
            logger.warn(format("Endpoint with id %s associated with composed node has not been found, on service with UUID: %s",
