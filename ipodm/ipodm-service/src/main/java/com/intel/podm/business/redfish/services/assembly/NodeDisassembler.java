...
public class NodeDisassembler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(NodeDisassembler.class);
    ...
    private EthernetSwitchPort getEthernetSwitchPortFromMac(MacAddress mac) {
        try {
            ...
            logger.error(format("Could not get neighbor switch port from mac address '%s'.", mac), e);
            return null;
        }
    }
}
